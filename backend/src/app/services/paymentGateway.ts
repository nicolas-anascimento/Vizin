...
export type GatewayResult = { referencia: string; status: "pendente" | "pago" | "falhou" | "cancelado" | "estornado"; codigo_copia_cola?: string; qrcode_url?: string; expira_em?: string };
export function simulated(): boolean { return env.NODE_ENV === "dev" && env.PAYMENT_MODE === "demo"; }
// Adapter HTTP: provider-specific credentials and SDK remain outside rental business rules.
export async function gatewayRequest(operation: string, data: Record<string, unknown>, key: string): Promise<GatewayResult> {
