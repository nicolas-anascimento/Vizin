...
// navegador.
const CHAVE_RASCUNHO = `rascunho_cadastro_objeto_${usuarioLogado?.email || "anonimo"}`;
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    // ============================================================
    // SIMULAÇÃO (MOCK) — sem back-end ainda.
    // Redimensiona/comprime as fotos escolhidas e salva o objeto via
    // window.ObjetosVizin.criar (localStorage), pra já poder testar
    // o fluxo completo: cadastrar -> aparecer em "Meus Objetos" ->
    // aparecer em "Produto".
    //
    // PONTO DE INTEGRAÇÃO COM O BACK-END: quando a API estiver pronta,
    // troque este bloco por um fetch de verdade, por exemplo:
    //
    // const formData = new FormData();
    // formData.append("titulo", ...);
    // ...
    // selectedFiles.forEach(f => formData.append("fotos", f, f.name));
    // const response = await fetch("/api/objetos", { method: "POST", body: formData });
    // ============================================================
    const imagensBase64 = await Promise.all(selectedFiles.map(f => redimensionarImagem(f)));
 
    const novoObjeto = window.ObjetosVizin.criar({
    ...
    setTimeout(() => {
      window.location.href = `../Produto/index.html?id=${novoObjeto.id}`;
    }, 900);
 
  } catch (err) {
    console.error(err);
    // Mensagem específica quando o erro é de cota do localStorage (fotos
    // grandes demais somadas), em vez de sempre mostrar a mensagem
    // genérica.
