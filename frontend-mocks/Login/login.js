...
// =====================================================
// "BANCO" DE USUÁRIOS MOCK (localStorage)
// =====================================================
const usuariosMock = [
    {
        nome: "João",
...
function buscarUsuarioPorCpf(cpf) {
    ...
    const mockEncontrado = usuariosMock.find(u => somenteNumeros(u.cpf) === cpfLimpo);
    if (mockEncontrado) {
        return mockEncontrado;
    }
 
    const usuarios = obterUsuariosCadastrados();
    return usuarios.find(u => somenteNumeros(u.cpf) === cpfLimpo) || null;
}
...
cadastroForm.addEventListener("submit", (e) => {
    ...
    if (buscarUsuarioPorCpf(cpf)) {
        return mostrarErroEtapa("registerErro", "Este CPF já está cadastrado");
    }
...
document.getElementById("loginForm").addEventListener("submit", (e) => {
    ...
    setTimeout(() => {
        ...
        const usuarioEncontrado = buscarUsuarioPorCpf(cpf);
 
        // CPF não está cadastrado
        if (!usuarioEncontrado) {
            erro.textContent = "CPF não cadastrado";
            erro.classList.remove("sucesso");
            erro.classList.add("erro");
            return;
        }
        ...
        // CPF e senha corretos: acesso liberado
